...
namespace SPM_AgentService
{
    ...
    class TCP_Listener
    {
        ...
        public TCP_Listener(IPAddress input_localaddr, int input_port)
        {
        ...
        private void Init(IPAddress input_localaddr, int input_port)
        {
            ...
            SPM_AgentService.eventLog.WriteEntry("SPM Monitoring system Agent Server Starting on Port: " + Port.ToString(), EventLogEntryType.Information, 1005);
        ...
        private async void Listen()
        {
            while (true)
            {
                try
                {
                    ...
                    {
                        SPM_AgentService.eventLog.WriteEntry("TCP Flood Attack detected from IP: " + ClientIP.ToString() + " - banned for " + floodBanning_Minutes + " min" + "\n\nIf it is your own Simple Ping Monitor ip address, check the Encryption Key which is set on this host Agent service and Simple Ping Monitor Options. To change Encryption Key on the Agent on this host you must reinstall the Agent.", EventLogEntryType.Error, 1010);
                    ...
                    if (queryList.Count() < floodBanning_junk_Packet_count)
                    {
                        ...
                        try
                        {
                            if (stream.CanRead)
                            {
                                ...
                                try
                                {
                                    string inputcommand = MyEncryption.DecryptText(myCompleteMessage.ToString(), SPM_AgentService.Instance.Encryption_Key);
                                    string response = MyEncryption.EncryptText(commandProcessor.GetCachedAnswer(inputcommand), SPM_AgentService.Instance.Encryption_Key);
                ...
                {
                    Server.Stop();
                    SPM_AgentService.eventLog.WriteEntry("SPM Monitoring system Agent server is broken and now will restart...\n\nError:" + ex.Message, EventLogEntryType.Error, 1006);
        ...
        public void junkRequestIPs_Reset_Timer_Elapsed(object sender, ElapsedEventArgs args)
        {
            if (junkRequestIPs.Count() > 0)
            {
                    ...
                    
                {
                    ...
                    {
                        SPM_AgentService.eventLog.WriteEntry("Early banned Client IP: " + ip.ToString() + " - now is restored", EventLogEntryType.Information, 1011);
                    }
                }
